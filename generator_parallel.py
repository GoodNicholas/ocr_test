...
# ────────── MAIN ──────────
def main():
    # размер «куска» для воркера и граница train/val
    ...
    with mp.Pool(processes=N_PROC, initializer=worker_init) as pool:
        for batch in pool.imap_unordered(_wrap, tasks, chunksize=1):
            for fn, is_train, ann, pbbox in batch:
                img_path = OUT_IMG_DIR / fn            # «сырой» файл-заглушка
                img      = Image.open(img_path)
                W, H     = img.size                   # нужны для stage-1
                ...
                if GENERATE_STAGE1:
                    ...
                    shutil.move(img_path, dst_img / fn)        # перенос
                    save_yolo_txt(
                        dst_lbl / f"{fn[:-4]}.txt",
                        [(0, tuple(pbbox))],                   # весь паспорт
                        (W, H)
                    )
                ...
                if GENERATE_STAGE2:
                    ...
                    # 1) сохраняем ПОЛНУЮ картинку (фон + паспорт)
                    img.save(dst_img / fn)

                    # 2) метки: bbox'ы уже содержат dx,dy → берём их как есть
                    items = [
                        (FIELD_CLASSES[a["label"]], tuple(a["bbox"]))
                        for a in ann
                    ]
                    save_yolo_txt(
                        dst_lbl / f"{fn[:-4]}.txt",
                    ...
                    # 3) DEBUG-визуализация: зелёные прямоугольники по рассчитанным координатам
                    if DEBUG:
                        dbg  = img.copy()
                        draw = ImageDraw.Draw(dbg)
                ...
                # если stage-1 выключен — удаляем «сырой» скриншот-заглушку
                if (not GENERATE_STAGE1) and img_path.exists():
                    img_path.unlink()

    # итоговая «сводка» в консоли
    print("✅ Датасет готов!")
    if GENERATE_STAGE1:
