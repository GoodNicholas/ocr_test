...
class OCRReader private constructor(
    ...
    fun setPageSegMode(psm: Int) {
        this.psm = psm
        tesseract.setPageSegMode(psm)
    }

    fun read(source: ImageSource): String = when (source) {
        is ImageSource.Path -> tesseract.doOCR(File(source.path))
        is ImageSource.ByteArraySource -> tesseract.doOCR(javax.imageio.ImageIO.read(source.data.inputStream()))
        is ImageSource.Stream -> tesseract.doOCR(javax.imageio.ImageIO.read(source.stream))
        is ImageSource.Buffered -> tesseract.doOCR(source.image)
    }

    fun read(path: String): String = read(ImageSource.Path(path))
    fun read(data: ByteArray): String = read(ImageSource.ByteArraySource(data))
    fun read(stream: InputStream): String = read(ImageSource.Stream(stream))
    fun read(image: BufferedImage): String = read(ImageSource.Buffered(image))

    class Builder {
        private var tessDataPath: String = "src/main/resources/tessdata"
